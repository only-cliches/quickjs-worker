...
fn process_globals<'a, C: Context<'a>>(
    ...
    if val.is_a::<JsFunction, _>(cx) {
        ...
        global_ptrs.push(("root".to_string(), GlobalTypes::Function { value: func.root(cx) }));
...
fn create_worker(mut cx: FunctionContext) -> JsResult<JsObject> {
    ...
    let kill_fn = JsFunction::new(&mut cx, move |mut cxf: FunctionContext| {
        ...
        **closed = true;
        ...
        send_fn.blocking_send(QuickChannelMsg::Quit { def: deferred }).unwrap_or(());
        return Ok(promise);
    }).unwrap();
    ...
    let on_fn = JsFunction::new(&mut cx, move |mut cxf: FunctionContext| {
        ...
        match on_type.as_str() {
            "message" => {
                ...
                send_fn.blocking_send(QuickChannelMsg::NewCallback {
    ...
    let memory_fn = JsFunction::new(&mut cx, move |mut cxf: FunctionContext| {
        ...
        let send_fn = QUICKJS_SENDERS.blocking_lock()[channel_id].clone();
        let (deferred, promise) = cxf.promise();
        send_fn.blocking_send(QuickChannelMsg::Memory { def: deferred }).unwrap_or(());
        return Ok(promise);
    }).unwrap();
    ...
    let gc_fn = JsFunction::new(&mut cx, move |mut cxf: FunctionContext| {
        ...
        let send_fn = QUICKJS_SENDERS.blocking_lock()[channel_id].clone();
        let (deferred, promise) = cxf.promise();
        send_fn.blocking_send(QuickChannelMsg::GarbageCollect {
                def: deferred,
                start: Instant::now(),
            }).unwrap_or(());
        return Ok(promise);
    ...
    let eval_fn = JsFunction::new(&mut cx, move |mut cxf: FunctionContext| {
        ...
        send_fn.blocking_send(QuickChannelMsg::EvalScript {
    ...
    let set_global = JsFunction::new(&mut cx, move |mut cxf: FunctionContext| {
        ...
        send_fn.blocking_send(QuickChannelMsg::SetGlobal {
                key,
    ...
    let to_byte_code = JsFunction::new(&mut cx, move |mut cxf: FunctionContext| {
        ...
        let script = cxf.argument::<JsString>(0)?.value(&mut cxf);
        let (deferred, promise) = cxf.promise();
        send_fn.blocking_send(QuickChannelMsg::GetByteCode {
                source: String::from(script),
                def: deferred,
            }).unwrap();
        return Ok(promise);
    }).unwrap();
    return_obj.set(&mut cx, "getByteCode", to_byte_code).unwrap();
    ...
    let to_byte_code = JsFunction::new(&mut cx, move |mut cxf: FunctionContext| {
        ...
        for i in 0..vec_size {
            let value = bytes.get_value(&mut cxf, i as u32).unwrap()
                .downcast::<JsNumber, _>(&mut cxf).unwrap();
            buffer.push(value.value(&mut cxf) as u8);
        }
        let (deferred, promise) = cxf.promise();
        send_fn.blocking_send(QuickChannelMsg::LoadByteCode {
                bytes: buffer,
                def: deferred,
            }).unwrap();
        return Ok(promise);

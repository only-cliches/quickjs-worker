...
use neon::prelude::*;
use neon::types::{Deferred, JsBuffer, JsDate, JsNull};
...
lazy_static! {
    ...
    static ref QUICKJS_CALLBACKS: Arc<tokio::sync::Mutex<Vec<Vec<JsValueFacade>>>> =
        Arc::new(tokio::sync::Mutex::new(Vec::new()));
    // (MaxInterrupt, CurrentInterrupt, DeadlineTimestamp)
    static ref INT_COUNTERS: Arc<tokio::sync::Mutex<Vec<(i64, i64, u128)>>> =
        Arc::new(tokio::sync::Mutex::new(Vec::new()));
    static ref GLOBAL_PTRS: Arc<tokio::sync::Mutex<Vec<Vec<(String, GlobalTypes)>>>> =
        Arc::new(tokio::sync::Mutex::new(Vec::new()));
    // [ADDED] Track pending async operations per channel
    static ref PENDING_OPS: Arc<tokio::sync::Mutex<Vec<usize>>> =
        Arc::new(tokio::sync::Mutex::new(Vec::new()));
}

/// Result of evaluating code in the QuickJS runtime.
///
/// - `Fulfilled` carries the resolved value (including non-Promise results).
/// - `Rejected` carries the rejection value (which may be an Error or any JS value).
#[derive(Debug, Clone)]
pub enum EvalOutcome {
    Fulfilled(JsDataTypes),
    Rejected(JsDataTypes),
}

impl EvalOutcome {
    #[inline]
    pub fn fulfilled(v: JsDataTypes) -> Self {
        Self::Fulfilled(v)
    }

    #[inline]
    pub fn rejected(v: JsDataTypes) -> Self {
        Self::Rejected(v)
    }
}
...
pub enum SyncEvalRequest {
    Execute {
        ...
        resp: oneshot::Sender<EvalOutcome>,
    },
    Console {
        level: String,
        args: Vec<JsDataTypes>,
    ...
    Done {
        result: (EvalOutcome, cpu_time::ProcessTime, Instant),
    },
}
...
pub enum QuickChannelMsg {
    ...
    ResolveGlobalPromise {
        id: u32,
        result: JsDataTypes,
        // CHANGED: From Option<String> to Option<JsDataTypes>
        error: Option<JsDataTypes>,
    },
    NewCallback {
...
fn process_globals<'a, C: Context<'a>>(
    ...
    if val.is_a::<JsFunction, _>(cx) {
        ...
        global_ptrs.push((
            "root".to_string(),
            GlobalTypes::Function {
                value: func.root(cx),
            },
        ));
        return json!({ "__t": "f", "__i": id });
    }
    ...
    {
        let obj = val.downcast::<JsObject, _>(cx).unwrap();
        let mut map = serde_json::Map::new();
        let keys = obj
            .get_own_property_names(cx)
            .unwrap_or(JsArray::new(cx, 0));
        let len = keys.len(cx);
        ...
        for i in 0..len {
            ...
            if let Ok(key_str_obj) = key_val.downcast::<JsString, _>(cx) {
                ...
                let sub_val: Handle<JsValue> = obj.get(cx, key_val).unwrap();
                let sub_json = process_globals(cx, sub_val, global_ptrs);
    ...
    let id = global_ptrs.len();
    global_ptrs.push(("root".to_string(), GlobalTypes::Data { value: data }));
    return json!({ "__t": "d", "__i": id });
}
...
fn create_worker(mut cx: FunctionContext) -> JsResult<JsObject> {
    ...
    if let Some(args_obj) = cx.argument_opt(0) {
        // ... (Argument parsing omitted for brevity, same as original) ...
        if let Ok(args_obj) = args_obj.downcast::<JsObject, _>(&mut cx) {
            ...
            if let Ok(value) = args_obj.get_value(&mut cx, "globals") {
                if let Ok(globals) = value.downcast::<JsObject, _>(&mut cx) {
                    ...
                    let structure_json = process_globals(&mut cx, globals.upcast(), target_vec);
    ...
    let post_message_fn = JsFunction::new(&mut cx, move |mut cxf: FunctionContext| {
        ...
        quick_chnl
            .blocking_send(QuickChannelMsg::SendMessageToQuick { message: msg })
            .unwrap_or(());
        return Ok(cxf.undefined());
    })
    .unwrap();
    return_obj
        .set(&mut cx, "postMessage", post_message_fn)
        .unwrap();
    ...
    let kill_fn = JsFunction::new(&mut cx, move |mut cxf: FunctionContext| {
        ...
        **closed = true;
        ...
        // Just send the Quit signal. Do NOT unref here.
        send_fn
    ...
    // 2. ADD this new 'unref' function
    // This allows JS to manually drop the event loop handle when it's ready.
    let unref_fn = JsFunction::new(&mut cx, move |mut cxf: FunctionContext| {
        let mut channels = NODE_CHANNELS.blocking_lock();
        if let Some(channel) = &mut channels[channel_id] {
            channel.unref(&mut cxf);
        }
        return Ok(cxf.undefined());
    })
    .unwrap();
    return_obj.set(&mut cx, "unref", unref_fn).unwrap();
    ...
    // eval_sync_fn does NOT need reference_loop because it blocks the main thread
    let closed_clone = is_closed.clone();
    let eval_sync_fn = JsFunction::new(&mut cx, move |mut cxf: FunctionContext| {
        if *closed_clone.lock().unwrap() == true {
            return cxf.throw_error("Runtime is closed");
        }
        // ... (rest of eval_sync_fn is unchanged) ...
        let send_fn = QUICKJS_SENDERS.blocking_lock()[channel_id].clone();
        let script = cxf.argument::<JsString>(0)?.value(&mut cxf);
        let (script_name, is_module, args) = parse_eval_options(&mut cxf, 1);
        let (tx, mut rx) = tokio::sync::mpsc::channel(1);

        // No ref loop needed here
        send_fn
            .blocking_send(QuickChannelMsg::EvalScript {
        ...
        let result = loop {
            if let Some(msg) = rx.blocking_recv() {
                            ...
                            match fn_handle.apply::<JsValue, _>(&mut cxf) {
                                Ok(val) => {
                                    if let Ok(str_val) = val.downcast::<JsString, _>(&mut cxf) {
                                        ...
                                        let _ = resp.send(Err(
                                            "Import callback must return a string".to_string(),
                                        ));
                                    }
                                }
                                Err(_) => {
                                    let _ = resp
                                        .send(Err("Import callback threw an error".to_string()));
                                }
                            }
                        } else {
                    ...
                    SyncEvalRequest::Execute {
                        ...
                        let execution_result = match value {
                            GlobalTypes::Function { value } => {
                                ...
                                match fn_handle.apply::<JsValue, _>(&mut cxf) {
                                    Ok(res) => JsDataTypes::from_node_value(res, &mut cxf)
                                        .map_err(|_e| "Conversion error".to_string()),
                        ...
                        let outcome = match execution_result {
                            Ok(v) => EvalOutcome::Fulfilled(v),
                            Err(e) => EvalOutcome::Rejected(JsDataTypes::Error {
                ...
                break (
                    EvalOutcome::Rejected(JsDataTypes::Error {
                        name: "".to_string(),
                        message: "".to_string(),
                        stack: None,
                        code: None,
        ...
        match result {
            (EvalOutcome::Fulfilled(data), cpu_time, start_time) => {
            ...
            (EvalOutcome::Rejected(err_val), _cpu_time, _start_time) => match err_val {
                JsDataTypes::Error {
                    name,
                    message,
                    stack,
                    ...
                    let obj_val = cxf.string(name);
                    obj.set(&mut cxf, "name", obj_val)?;
                    if let Some(stack) = stack {
                        let stack_val = cxf.string(stack);
                        obj.set(&mut cxf, "stack", stack_val)?;
                    }
                    if let Some(code) = code {
                        let code_val = cxf.string(code);
                        obj.set(&mut cxf, "code", code_val)?;
                    }
                    let obj_value = obj.as_value(&mut cxf);
                    return cxf.throw(obj_value);
                }
                other => cxf.throw_error(other.to_string()),
            },
        }
    })
    .unwrap();
    ...
    let set_global = JsFunction::new(&mut cx, move |mut cxf: FunctionContext| {
        ...
        let val = cxf.argument::<JsValue>(1)?;
        let (deferred, promise) = cxf.promise();

        let mut globals_vec = GLOBAL_PTRS.blocking_lock();
        let target_vec = &mut globals_vec[channel_id];
        let structure_json = process_globals(&mut cxf, val, target_vec);
        let structure_str = structure_json.to_string();
    ...
    let closed_clone = is_closed.clone();
    let load_byte_code = JsFunction::new(&mut cx, move |mut cxf: FunctionContext| {
    ...
    .unwrap();
    return_obj.set(&mut cx, "loadByteCode", load_byte_code)?;

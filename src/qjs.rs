            ...
            if let Some(inner_obj) = val.as_object() {
                if inner_obj.contains_key("__t") {
                    ...
                    if type_str == "f" {
                         ...
                         script.push_str(&format!("{} = __INTERNAL_CALL_GLOBAL({}, []);\n", current_access, id));
                    }
                } else {
                    if path.is_empty() {
        ...
        match script_result {
            ...
            Ok(mut script_result) => {
                ...
                let js_data_type = rt_clone.lock().await.loop_async(move |rt| {
                    ...
                    // CHECK FOR MODULE RETURN
                    if is_module {
                        if let Ok(ret_val) = realm.get_global().and_then(|g| realm.get_object_property(&g, "__qjs_module_return_value")) {
                            if !ret_val.is_undefined() {
                                // Clear it
                                let _ = realm.get_global().and_then(|g| realm.set_object_property(&g, "__qjs_module_return_value", &realm.create_undefined().unwrap()));
                ...
                match msg {
                    SyncChannelMsg::ProcessAsync { future, tx } => {
                        match tokio_rt.block_on(future) {
                            ...
                            Err(_e) => { tx.send(JsDataTypes::Undefined).unwrap(); }
                        }
                    },
                    SyncChannelMsg::RespondEval { def, result, start_time, cpu_time } => {
                        ...
                        if let Some(channel) = channel {
                            channel.send(move |mut cx| {
                                ...
                                let interrupt_count = if int_counters[channel_id].0 > 0 { cx.number(int_counters[channel_id].1 as f64) } else { cx.number(-1) };
                                let total_cpu = cpu_time.elapsed().as_nanos();
                                let eval_time = cx.number(start_time.elapsed().as_nanos() as f64 / (1000f64 * 1000f64));
                            ...
                            channel.send(move |mut cx| {
                                ...
                                for cbb in unlocked.iter() {
                                    if cbb.1 == NodeCallbackTypes::Message {
                                        // FIX: Use &mut cx and expect JsValue (ignore result)
                                        let _ = cbb.0.to_inner(&mut cx).call_with(&mut cx).arg(out).apply::<JsValue, TaskContext<'_>>(&mut cx)?;
                                    }
                                }
                                Ok(())
                            });
                    ...
                    SyncChannelMsg::Memory { json, def } => {
                        ...
                        if let Some(channel) = node_channel {
                            channel.send(move |mut cx| {
                                let json_parse = cx.global::<JsObject>("JSON")?.get_value(&mut cx, "parse")?.downcast::<JsFunction, _>(&mut cx).unwrap();
                                let out = json_parse.call_with(&mut cx).arg(cx.string(json)).apply::<JsObject, _>(&mut cx)?;
                    ...
                    SyncChannelMsg::SendBytes { bytes, def } => {
                        ...
                        if let Some(channel) = &unlocked[channel_id] {
                            channel.send(move |mut cx| {
                                let json_value = QuickJSWorker::vec_u8_to_node_array(&bytes, &mut cx).unwrap();
                            ...
                            channel.send(move |mut cx| {
                                ...
                                for cbb in unlocked_cbs.iter() {
                                    if cbb.1 == NodeCallbackTypes::Close {
                                        // FIX: Use &mut cx and expect JsValue (ignore result)
                                        let _ = cbb.0.to_inner(&mut cx).call_with(&mut cx).apply::<JsValue, TaskContext<'_>>(&mut cx)?;
                                    }
                                }
        ...
        tokio_rt.block_on(async move {
            ...
            while let Some(message) = urx.recv().await {
                ...
                tokio::spawn(async move {
                    let msg_cb_clone = msg_cb_clone.clone();
                    match message {
                        QuickChannelMsg::SetGlobal { key: _ , structure, def } => {
                            ...
                            let init_script = runtime.loop_async(move |_runtime| {
                                ...
                                let generated_code = generate_init_script(&structure_json, String::new());
                            ...
                            let init_script = runtime.loop_async(move |runtime| {
                                ...
                                realm.install_function(&[], "__INTERNAL_CALL_GLOBAL", |_runtime, realm, _this, args| {
                                    ...
                                    if let Some(sync_sender) = sync_sender_opt {
                                        ...
                                        let req = crate::SyncEvalRequest::Execute {
                                            callback_id,
                                            args: js_args,
                                            resp: resp_tx
                                        };
